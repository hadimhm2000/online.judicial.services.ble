...
async def resilient_upload_attachment_groups(
    ...
    for idx, group in enumerate(groups):
        ...
        if not paths:
            # ══════════════════════════════════════════════════════════
            # قبلاً این حالت به‌اشتباه «موفق» ثبت می‌شد و بی‌صدا رد می‌شد؛
            # یعنی اگر دانلود تصاویر از بله شکست می‌خورد (مثلاً به‌دلیل
            # منقضی‌شدن file_id در فاصلهٔ طولانی بین ارسال تصویر توسط کاربر
            # و پردازش واقعی دسته‌جمعی پس از تایید مدیر)، هیچ پیوستی آپلود
            # نمی‌شد و هیچ خطایی هم دیده نمی‌شد. حالا این حالت شکست محسوب
            # و به کاربر/مدیر اطلاع داده می‌شود.
            # ══════════════════════════════════════════════════════════
            _log(prefix, f"گروه [{title}] فایلی ندارد (دانلود تصاویر ناموفق بوده) — شکست", 'warning')
            overall["success"] = False
            overall["failed_groups"].append({
                "title": title,
                "error": "هیچ فایلی برای این گروه پیوست دانلود نشد (احتمالاً لینک تصاویر بله منقضی شده)",
                "attempts": 0,
            })
            continue
