...
from panel_sync import mark_case_ready_to_send_by_tracking, mark_case_signed_by_tracking
...
async def on_lavayeh_sign_submit_success(bot: Bot, user_id: int, row_idx: int, state: FSMContext):
    ...
    if not persons_awaiting:
        ...
        # تا ستون «امضا» پنل ادمین به‌روز شود (قبلا همیشه «بدون امضا» می‌ماند).
        try:
            tracking_code = sign_info.get("tracking_code", "")
            svc_type = sign_info.get("service_type", "LAVAYEH")
            await mark_case_signed_by_tracking(user_id, svc_type, tracking_code)
        except Exception as panel_err:
            logging.warning(f"[SIGN] خطا در ثبت وضعیت امضای پرونده لایحه در پنل: {panel_err}")
        await state.clear()
    else:
...
async def on_ezhhar_sign_submit_success(bot: Bot, user_id: int, row_idx: int, state: FSMContext):
    ...
    if not persons_awaiting:
        ...
        try:
            ...
            logging.warning(f"[EZHHAR_SIGN] خطا در ثبت وضعیت امضای پرونده اظهارنامه در پنل: {panel_err}")
        await state.clear()
    else:
        all_persons = sign_info.get("sign_persons", [])
        remaining_names = []
