...
# =========================================================
# ⭐ پنجره‌های ۳۰ دقیقه‌ای ویرایش کدملی + جریمهٔ نصف پیش‌پرداخت
# (nid_fix_window.py — منبع واحد حقیقت؛ ماندگار در persistence)
# =========================================================
# کلید: user_id (int)
# مقدار: {
#   "flow": str,                — lavayeh / ezhharnameh / tn / check
#   "task_data": dict,          — اطلاعات کامل تسک (برای ویرایش + ارسال مجدد)
#   "error_text": str,          — متن خطای سامانه
#   "national_id": str,         — کدملی خطادار
#   "person_role": str,         — نقش شخص (اختیاری)
#   "person_index": int,        — ایندکس شخص در لیست (-1 = نامشخص)
#   "created_at": datetime,     — زمان شروع پنجره
#   "deadline": datetime,       — مهلت ۳۰ دقیقه‌ای
#   "penalty_applied": bool,
# }
pending_nid_fix_windows: dict = {}
...
# پس از پرداخت پیش‌پرداخت (nid_fix_window.py)
# =========================================================
# کلید: user_id (int)
# مقدار: {
#   "task_data": dict,          — اطلاعات کامل تسک (با task_type)
#   "error_text": str,
#   "created_at": datetime,
#   "deadline": datetime,       — مهلت ۴۵ دقیقه‌ای
# }
pending_tn_retrieve_fix: dict = {}

# =========================================================
# ⭐ پنجرهٔ ۴۵ دقیقه‌ای ویرایش «شماره قرارداد وکالت» (اصلاحیه ۱۴۰۵/۰۶)
# پس از خطای «شماره قرارداد الکترونیک وکالت «...» معتبر نمی باشد» در
# بخش منضمات (nid_fix_window.py — ماندگار در persistence)
# =========================================================
# کلید: user_id (int)
# مقدار: {
#   "flow": str,               — lavayeh / ealam / ezhharnameh / check / tn
#   "task_data": dict,         — اطلاعات کامل تسک اصلی (برای ادامهٔ ثبت)
#   "bill_no": str,            — کد رهگیری/بایگانی ثبت موقت در سامانه
#   "old_contract": str,       — شماره قرارداد نامعتبر
#   "stamp_amount_value": int, — مبلغ تمبر وکیل (ریال) — قبلاً محاسبه‌شده
#   "error_text": str,         — متن خطای سامانه
#   "created_at": datetime,
#   "deadline": datetime,      — مهلت ۴۵ دقیقه‌ای
# }
pending_contract_fix: dict = {}

# =========================================================
# درخواست‌های لایحه در انتظار ویرایش کدملی (alias پنجرهٔ ۳۰ دقیقه‌ای)
# =========================================================
pending_lavayeh_sana_fix: dict = {}
...
# ربات به‌جز استعلامات، پیش از شروع ثبت در سامانه دریافت می‌شود
# (لایحه/اظهارنامه: ۱,۰۰۰ تومان — سایر سرویس‌ها: ۲,۰۰۰ تومان).
# در پایان کار، هزینه کل اعلام و این مبلغ از آن کسر می‌شود و فاکتور
# مابقی برای کاربر ارسال می‌گردد.
# =========================================================
# کلید: user_id (int)
# مقدار: {
#   "amount_toman": int,     — مبلغ پیش‌پرداخت (تومان)
#   "amount_rial": int,      — مبلغ پیش‌پرداخت (ریال)
#   "service": str,          — کلید سرویس (lavayeh/ezhharnameh/tn/check)
#   "service_label": str,    — نام نمایشی سرویس
#   "charge_id": str,        — شناسه تراکنش کیف پول بله
#   "paid_at": datetime,     — زمان پرداخت
# }
prepaid_registrations: dict = {}
...
# =========================================================
# ⭐ کاربرانی که پیام بازیابی کرش برایشان ارسال شده (ارسال فقط یک‌بار)
# =========================================================
# کلید: user_id (int) | مقدار: اثر انگشت پرونده/رویداد (str)
recovery_notified: dict = {}

# =========================================================
# فرصت رایگان اصلاح «کد رهگیری نامعتبر / کد رهگیری متعلق به فرم دیگر»
# (۴۵ دقیقه، بدون پرداخت مجدد)
# =========================================================
# کلید: user_id (int)
# مقدار: {
#   "expires_at": datetime,     — پایان مهلت ۴۵ دقیقه‌ای
...
INVALID_TRACKING_RETRY_MINUTES = 45
