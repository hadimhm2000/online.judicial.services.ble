...
async def _navigate_to_sign_page_once(
    ...
    try:
        ...
        # ⭐ اصلاحیه (دعاوی اعتراضی): زیرمنوهای «دعاوی اعتراضی»
        # (تجدیدنظرخواهی، واخواهی، فرجام خواهی، اعاده دادرسی مدنی/کیفری،
        # اعتراض ثالث، اعتراض به قرار دادسرا) li.list-group-item هستند و
        # داخل ul جداگانه‌ای زیر هدر <a class="list-group-item"> به نام
        # «دعاوی اعتراضی» قرار دارند — پس ابتدا هدر اصلی باز می‌شود و
        # سپس همان زیرمنویی که کاربر ثبت را با آن شروع کرده (عیناً مثل
        # مسیر ثبت اظهارنامه/تجدیدنظر).
        OBJECTION_SUB_MENUS = {
        ...
        if first_menu_text in OBJECTION_SUB_MENUS:
            # ۱) اگر زیرمنو visible باشد مستقیم کلیک شود؛ وگرنه هدر
            #    «دعاوی اعتراضی» باز می‌شود و پس از صبر، زیرمنو کلیک می‌شود
            state = await sana_page.evaluate('''(text) => {
                ...
                if (r.width > 0 && r.height > 0) { t.click(); return "clicked"; }
                const heads = Array.from(document.querySelectorAll('a.list-group-item'));
                const head = heads.find(a => (a.innerText || "").includes("دعاوی اعتراضی"));
                if (head) { head.click(); return "expanded"; }
                return "no_header";
            }''', first_menu_text)

            if state == "not_found":
                logging.error(f"[SIGN] زیرمنوی «{first_menu_text}» پیدا نشد")
                return False
            if state == "no_header":
                logging.error("[SIGN] منوی اصلی «دعاوی اعتراضی» پیدا نشد")
                return False
            if state == "expanded":
                await resilient_sleep(sana_page, 3, bot, user_id)
                # حالا که زیرمنو visible است، همان گزینهٔ شروعِ ثبت کلیک می‌شود
        ...
        for sub_menu_text in sub_menus:
