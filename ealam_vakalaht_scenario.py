...
async def _upload_electronic_vakalaht(
    ...
    for attempt in range(3):
        try:
            ...
            # خالی (حالت عادی اعلام وکالت) تغییر نکند.
            # ══════════════════════════════════════════════════════════
            ...
            if form_dirty:
                ...
                if clicked_new:
                    logging.info("[EALAM][منضمات] کلیک «پیوست جدید» قبل از وکالت‌نامه الکترونیک (فرم در حالت ویرایش بود)")
                    await asyncio.sleep(3)
                    await wait_for_angular_idle(page)
                    await asyncio.sleep(1)
                else:
                    logging.warning("[EALAM][منضمات] فرم پر بود ولی دکمهٔ «پیوست جدید» پیدا نشد")
            ...
            if not select_result.get("found"):
                logging.warning(f"[EALAM] گزینه «تصویر الکترونیک وکالت نامه» یافت نشد (تلاش {attempt+1}): {select_result.get('reason')}")
                await asyncio.sleep(5)
                continue
            logging.info(f"[EALAM] انتخاب نوع پیوست وکالت‌نامه: روش={select_result.get('method')}, نشست={select_result.get('stuck')}")
            if not select_result.get("stuck"):
                logging.warning(f"[EALAM] انتخاب نوع پیوست ماندگار نشد (تلاش {attempt+1}) — تلاش مجدد")
                await asyncio.sleep(3)
                continue
            selected = True
            ...
            if contract_number:
                filled_no = await fill_input_angular(page, "#txtNo", contract_number, prefix="EALAM")
                if not filled_no:
                    logging.warning(f"[EALAM][منضمات] شماره قرارداد در #txtNo نماند — تلاش مجدد (تلاش {attempt+1})")
                    await asyncio.sleep(2)
                    continue
                await asyncio.sleep(1)
            ...
            if lawyer_amount_value and lawyer_amount_value > 0:
                filled_amount = await fill_input_angular(page, "#txtLawyerAmount", lawyer_amount_value, prefix="EALAM")
                if not filled_amount:
                    logging.warning(f"[EALAM][منضمات] مبلغ حق‌الوکاله در #txtLawyerAmount نماند — تلاش مجدد (تلاش {attempt+1})")
                    await asyncio.sleep(2)
                    continue
                await asyncio.sleep(1)
            else:
...
async def _print_lavayeh(page, browser_context, bill_no: str, bot: Bot, user_id: int) -> str:
    ...
    pdf_path = temp_path(f"ealam_vakalaht_{_code}.pdf")
...
async def _download_images_from_bale(bot: Bot, file_ids: list, user_id: int) -> list:
    ...
    ⭐ قبلاً نسخه محلی (ealam_img_*) بدون هیچ فشرده‌سازی‌ای بود؛ برای
    ...
    return await download_images_from_bale(bot, file_ids, user_id, prefix="EALAM")
