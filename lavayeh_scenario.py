        ...
        if has_service_delay:
            ...
            if service_delay_count >= 2:
                ...
                raise SanaSystemDownError(
                    "استعلام اشخاص لایحه: service_delay persisted")
            await asyncio.sleep(3)
            continue
...
async def _click_save_temp_with_retry(page, bot: Bot, user_id: int, max_retries: int = 5):
    ...
    for attempt in range(max_retries):
        ...
        if error_text:
            ...
            # ⭐ طبق دستور کارفرما: «تاخیر در اجرای سرویس» خطای موقت سامانه
            # است، نه خطای قطعی داده — تا ۲ بار تلاش مجدد (کلیک دوبارهٔ
            # «ثبت موقت»)؛ اگر بعد از آن هم برطرف نشد، به کاربر اطلاع داده
            # می‌شود که سامانه قطع است.
            if ("تاخیر در اجرای سرویس" in error_text or "سرویس با خطا" in error_text or
                ...
                if service_delay_count >= 2:
                    ...
                    try:
                        ...
                        pass
                    raise SanaSystemDownError("ثبت موقت لایحه: service_delay persisted")
                await asyncio.sleep(3)
                continue
    ...
    for attempt in range(max_retries):
        ...
        for _ in range(15):  # حداکثر ۷.۵ ثانیه انتظار
            ...
            if popup_info['type'] == 'confirm_prep':
                ...
                if is_concurrent:
                    logging.warning(f"[LAVAYEH] ورود همزمان/انقضای نشست قبل از تأیید آماده‌سازی (تلاش {attempt+1}) — تمدید نشست")
                    await handle_session_expired(bot, user_id, page=page)
                    await asyncio.sleep(3)
                    break  # خروج از حلقهٔ داخلی → تلاش مجدد کلیک #btnPreparation
        ...
        for _ in range(15):  # حداکثر ۷.۵ ثانیه
            result = await page.evaluate('''() => {
                ...
                if (!popup) return null;
                const h2 = popup.querySelector('h2');
                const pEl = popup.querySelector('p');
                const h2Text = ((h2 ? h2.innerText.trim() : '') + ' ' + (pEl ? pEl.innerText.trim() : '')).trim();
                const successIcon = popup.querySelector('.sa-icon.sa-success');
            ...
            if result['type'] == 'success':
                ...
                is_concurrent = await detect_concurrent_login_popup(page)
                if is_concurrent:
                    logging.warning(f"[LAVAYEH] ورود همزمان/انقضای نشست در آماده‌سازی (تلاش {attempt+1}) — تمدید نشست")
                    await handle_session_expired(bot, user_id, page=page)
                    await asyncio.sleep(3)
                    break  # retry از ابتدا پس از تمدید نشست
